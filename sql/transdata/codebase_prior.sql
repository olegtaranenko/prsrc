...
begin
	...
	if isnull(p_id, 0) = 0 then	
		set wf_breadcrump = null;
		return;
	end if;

	select klassname, parentklassid into wf_breadcrump, v_parentid from sguideklass where klassid = p_id;
	if isnull(v_parentid, 0) != 0 then
			set wf_breadcrump =  wf_breadcrump(v_parentid, p_table_name, p_pk_column_name, p_text_column_name) + ' / ' + wf_breadcrump;
--		set v_parent_name = ;
--		if v_parent_nam is not null then
--		end if;
	end if;
...
create table #aparent (parent integer, child_count integer, lvl integer);
--create table #ordered (id integer, ord integer);
	...
	branch: loop
		insert into #aparent
			...
			from sguideklass c
			join #aparent p on c.klassid = p.parent
	...
	for schich0 as s0 dynamic scroll cursor for
        ...
        from sguideklass k
        left join #aparent p on p.parent = k.klassid and p.lvl = v_lvl + 1
        where parentklassid = 0 and klassid != 0 order by klassname
	do
    
        insert into #ordered (id, ord) select r_klassid, v_cur_pos;
	...
	branch2: loop
		...
		for schich1 as s1 dynamic scroll cursor for
            ...
            from sguideklass k
            join #ordered o on o.id = k.parentklassid
            left join #aparent p on k.klassid = p.parent and p.lvl = v_lvl + 1
            where not exists (select 1 from #ordered o1 where o1.id = k.klassid)
...
--	select * from #aparent where lvl = v_lvl + 1;
...
select trim(n.cod + ' ' + nomname + ' ' + n.size) as name, s.shipped, s.sm, s.nomnom, o.ord, k.klassid, wf_breadcrump(k.klassid) as klassname, n.cost, n.ed_izmer2
from #nomenk_saled s
join sguidenomenk n on n.nomnom = s.nomnom
join #ordered o on o.id = n.klassid
