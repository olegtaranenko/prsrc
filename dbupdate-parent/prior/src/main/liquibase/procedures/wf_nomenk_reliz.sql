...
--	create table #klass_ordered (id integer, ord integer);
	...
	insert into #izdelia_reliz
	    select 
    		...
    		, sum(p.cenaEd * po.quant) as cenaTotal
    		, sum(io.costEd * po.quant) as costTotal
		...
		join orders o on o.numorder = po.numorder
		WHERE 
			po.outDate between 
					isnull(p_start, '20010101') 
				and isnull(p_end, '21001231')
			and o.werkId = p_werkId
		group by po.prid
	;
